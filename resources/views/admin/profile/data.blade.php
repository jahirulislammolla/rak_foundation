{{ $profiles->links('components.paginator') }}
...
<div class="grid gap-4 md:grid-cols-1 print:grid-cols-1 rounded-lg mt-3 px-1">
    <table class="w-full">
        ...
        <tbody>
            ...
            <tr class="text-center">
                <td class="{{ $class_body_tr }}">{{ $profile->id ?? ''}}</td>
                <td class="{{ $class_body_tr }}">{{ $profile->title ?? ''}}</td>
                <td class="{{ $class_body_tr }}">{{ $profile->type == '1' ? 'Academic Profile' : 'Research and Professional Profile'}}</td>
                <td class="{{ $class_body_tr }} text-left">{!! $profile->content ?? '' !!}</td>
                <td class="{{ $class_body_tr }}">{{ $profile->priority ?? ''}}</td>
                <td class="{{ $class_body_tr }}">{{ $profile->status == 1 ? 'Actice':'InActive'}}</td>
                <td class="{{ $class_body_tr }}">
                    <div class="flex justify-between gap-1">
                        <a href="{{ route('manage-profiles.edit', $profile->id) }}"
                            class="edit_profile px-3 py-1 rounded bg-sky-500 hover:bg-blue-400 text-white">
                            Edit
                        </a>
                    </div>
                </td>
