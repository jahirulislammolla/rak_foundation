{{ $leaderships->links('components.paginator') }}
...
<div class="grid gap-4 md:grid-cols-1 print:grid-cols-1 rounded-lg mt-3 px-1">
    <table class="w-full">
        ...
        <tbody>
            ...
            <tr class="text-center">
                <td class="{{ $class_body_tr }}">{{ $leadership->id ?? ''}}</td>
                <td class="{{ $class_body_tr }}">{{ $leadership->title ?? ''}}</td>
                <td class="{{ $class_body_tr }} text-start">{!! $leadership->content ?? '' !!}</td>
                <td class="{{ $class_body_tr }}">{{ $leadership->priority ?? ''}}</td>
                <td class="{{ $class_body_tr }}">{{ $leadership->status == 1 ? 'Actice':'InActive'}}</td>
                <td class="{{ $class_body_tr }}">
                    <div class="flex justify-between gap-1">
                        <a href="{{ route('manage-leaderships.edit', $leadership->id) }}"
                            class="edit_leadership px-3 py-1 rounded bg-sky-500 hover:bg-blue-400 text-white">
                            Edit
                        </a>
                    </div>
                </td>
