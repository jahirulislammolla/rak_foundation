...
<div class="py-2 flex justify-between align-center">
    ...
    <div class="text-left">
        ...
        <input type="text" 
        name="search_professional_service" 
        id="search_professional_service"
        value="{{ request()->input('search') ?? '' }}"
        class="px-4 py-1.5 rounded border border-slate-900 text-black"
        placeholder="search professional_service..."
        />
    </div>
</div>
...
@include('admin.professional_service.form', [
    'professional_service' => new \App\Models\ProfessionalService(),
...
<script type="text/javascript">
    ...
    const searchInput = document.getElementById('search_professional_service');
    ...
    function search(page = 1) {
        ...
        const url = new URL('/manage-professional-services', window.location.origin);
    ...
    $("body").on("keyup", "#search_professional_service", function(){
        search({{ request()->input('page') ?? 1 }})
    });
    ...
    $('input[name="professional_service_type"]').on('change', function(){
